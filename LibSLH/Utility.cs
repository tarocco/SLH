        ...
        BindingFlags flags)
        {
            ...
            if (member_path_elements.Count == 1)
            {
                switch (member_info.MemberType)
                {
                    case MemberTypes.Field:
                        if (set_value)
                            ((FieldInfo)member_info).SetValue(target, args[0]);
                        else
                            return ((FieldInfo)member_info).GetValue(target);
                        break;

                    case MemberTypes.Property:
                        if (set_value)
                            ((PropertyInfo)member_info).SetValue(target, args[0]);
                        else
                            return ((PropertyInfo)member_info).GetValue(target);
                        break;
                    ...
                    case MemberTypes.Method:
                        ...
                        var args_converted = parameters
                            ...
                            .Select(e => Convert(e.p.ParameterType, e.a))
                            .ToArray();
                        return method_info.Invoke(target, args_converted);
                }
                return args[0];
            }
            else
            {
                ...
                var next_member_path_elements = member_path_elements.Skip(1).ToList();
                var next_member_path = current_member_path + "." + next_member_path_elements.First();
                return EvalMemberPath(member_target, next_member_path_elements, next_member_path, args, set_value, flags);
            }
        }
        */
    }
